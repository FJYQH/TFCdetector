...
class TransformerVar(nn.Module):
    ...
    def __init__(self, win_size, enc_in, c_out, n_memory, shrink_thres=0, \
        ...
        self.gruunet = nn.Conv1d(in_channels=enc_in, out_channels=enc_in, kernel_size=3, padding=1)

        ######################### 频率 全局、局部学习
        self.atten = nn.ModuleList(  # d_model", default=256   d_inner", default=512  n_head", default=8
            [
                EncoderLayer_selfattn(256, 512, 8, 64, 64, dropout=0.3,
                                      # self.hp.d_model,
                                      # self.hp.d_inner,
                                      # self.hp.n_head,
    ...
    def forward(self, x):
        ...
        for i, series in enumerate(tmp_coefs):
            # if i ==1:
            #     series = winsorization(series)
            #     series = bilateral_filter(series)
            # if i==2:
            #     series = winsorization(series)
            series_unet = self.unet1d_model[i](series)
            new_coefs.append(series_unet)
        new_series_out = self.idwt((new_coefs[0], new_coefs[1:]))

        # 序列分解后，重建后 处理--频域 学习
        new_series_out = torch.unbind(new_series_out, dim=1)
